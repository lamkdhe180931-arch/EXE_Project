document.addEventListener('DOMContentLoaded', () => {
  ...
  if (pdpPage) {
    ...
    if (writeReviewBtn && reviewForm) {
      writeReviewBtn.addEventListener('click', () => {
        ...
        writeReviewBtn.textContent = isHidden ? 'Hủy viết đánh giá' : 'Viết đánh giá';
      });
    }

    // 6. Review Form Rating Input
    const starSelects = document.querySelectorAll('.pdp-rating-input .star-select');
    ...
    if (reviewForm) {
      reviewForm.addEventListener('submit', (e) => {
        e.preventDefault();
        alert('Cảm ơn bạn đã gửi đánh giá! Đánh giá của bạn đang được kiểm duyệt.');
        reviewForm.reset();
      ...
      q.addEventListener('click', () => {
        ...
        document.querySelectorAll('.pdp-faq-item').forEach(otherItem => {
          if (otherItem !== item && otherItem.classList.contains('active')) {
            ...
            otherItem.querySelector('.pdp-faq-question').setAttribute('aria-expanded', 'false');
  ...
  if (catalogueGrid) {
    ...
    const categoryButtons = document.querySelectorAll('#category-filters-container .filter-btn');
    const sortButtons = document.querySelectorAll('#price-sorts-container .sort-btn');
    ...
    const productCards = Array.from(catalogueGrid.querySelectorAll('.product-card'));
    ...
    function layoutMasonry() {
      ...
      const items = Array.from(catalogueGrid.querySelectorAll('.product-card:not(.filtered-out)'))
    ...
    function handlePageChange() {
      const activeItems = productCards.filter(card => !card.classList.contains('filtered-out'));
    ...
    function handleUrlParams() {
      ...
      if (categoryParam) {
        const targetBtn = Array.from(categoryButtons).find(btn => btn.getAttribute('data-category') === categoryParam.toLowerCase());
        if (targetBtn) {
          targetBtn.click();
  ...
  const artistCards = Array.from(document.querySelectorAll('.artist-wavy-card'));
  ...
  if (wavyContainer && wavyTrack && artistCards.length > 0) {
    ...
    const cardWidth = 280;
    let stepX = Math.max(360, Math.ceil((window.innerWidth + cardWidth) / artistCards.length));
    ...
    wavyContainer.addEventListener('touchstart', () => {
      isPaused = true;
    }, { passive: true });

    wavyContainer.addEventListener('touchend', () => {
      isPaused = false;
    }, { passive: true });
    ...
    function updateSlider() {
      ...
      cardsData.forEach(card => {
        ...
        const zIndex = Math.round(10 + scaleFactor * 90) + (card.isHovered ? 200 : 0);
    ...
    window.addEventListener('resize', () => {
      ...
      stepX = Math.max(360, Math.ceil((window.innerWidth + cardWidth) / artistCards.length));
